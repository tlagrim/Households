...
class CreateAssignmentViewController: UIViewController {
    ...
    @IBAction func showtest(sender: AnyObject) {
        print(existingUsersInActiveOccupancy)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!){
        if(segue.identifier == "ChildAssignment"){
            let childTVC = segue.destinationViewController as! AssignmentOptionsTVC
            childTVC.theParentVC = self
        }
        if(segue.identifier == "ChildMembers"){
            let childTVC = segue.destinationViewController as! MemberOptionsTVC
            childTVC.theParentVC = self
        }
    }
}
